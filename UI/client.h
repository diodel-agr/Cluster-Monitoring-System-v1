...
#define CLIENT_NAME				"Admin"
#define CLIENT_PASS				"admin_pass"
...
public:
     ...
     * Parameterized constructor.
     */
    ClientTCP(QString name, QString pwd, std::string addr, int port);
    ...
    /*
     * getLog - method used to obtain the log file from a client.
     *
     * @clientName: name of the client.
     * @file: filename.
     * @buffer: buffer to store partial file.
     * @max_size: buffer maximum size.
     * @return: the requested file or null if some error occured.
     */
    char* getLog(char* clientName, char* file, char* buffer, int max_size);
     ...
     */
    int getClientList(char* buffer, int size);

private:
    /*
     * QStringToString - method used to convert from a Qstring to a std::string.
     *
     * @qstr: QString variable to convert.
     * @return: the std::string representation of qsrt.
     */
